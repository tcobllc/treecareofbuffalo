...
(function () {
  ...
  /* ---------- count the rings: press and hold ---------- */
  var ringBtn = d.querySelector('.ring-btn');
  if (ringBtn) {
    var rings = ringBtn.querySelectorAll('.gr');
    var stats = d.querySelectorAll('.proof-stat');
    var hint = d.querySelector('.ring-stage .hint');
    var prog = 0, raf = 0, holding = false, done = false, last = 0;
    var per = 1 / rings.length;
    function paintRings() {
      rings.forEach(function (r, i) {
        var p = Math.max(0, Math.min(1, (prog - i * per) / per));
    ...
    function finishRings() {
      ...
      stats.forEach(function (st, i) {
        setTimeout(function () { st.classList.remove('dim'); }, i * 260);
      });
      if (hint) hint.textContent = 'Ten rings. Ten years and counting.';
    }
    function tickRings(t) {
      var dt = Math.min((t - last) / 1000, 0.05); last = t;
      prog = Math.max(0, Math.min(1, prog + (holding ? dt / 2.2 : -dt / 1.1)));
      paintRings();
      if (prog >= 1 && !done) { finishRings(); return; }
      if ((holding || prog > 0) && !done) raf = requestAnimationFrame(tickRings);
    }
    function holdStart(e) {
      if (done) return;
      e.preventDefault();
      holding = true; last = performance.now();
      cancelAnimationFrame(raf); raf = requestAnimationFrame(tickRings);
    }
    function holdStop() { holding = false; }
    if (reduce.matches || navigator.webdriver) {
      prog = 1; paintRings(); finishRings();
    } else {
      stats.forEach(function (st) { st.classList.add('dim'); });
      ringBtn.addEventListener('pointerdown', holdStart);
      addEventListener('pointerup', holdStop);
      ringBtn.addEventListener('pointercancel', holdStop);
      ringBtn.addEventListener('keydown', function (e) {
        if ((e.key === ' ' || e.key === 'Enter') && !e.repeat) holdStart(e);
      });
      ringBtn.addEventListener('keyup', function (e) {
        if (e.key === ' ' || e.key === 'Enter') holdStop();
      });
    }
  }
})();
